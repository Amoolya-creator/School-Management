    ...
    onValue(ref(db,Path), (snap) => {
        ...
        if (snap.exists()) {
            ...
            for (var Individual in Manpower){
                ...
                tt +=  '<div class="row form-group">\
                <label class="col form-label">'+data.Post+'</label>\
                <label class="col form-label">'+data.Name+'</label>\
                <label class="col form-label">'+data.UserID+'</label>\
...
$("#Prints").on('click',()=>{
    print();
    window.localStorage.setItem("Printed",true)
})